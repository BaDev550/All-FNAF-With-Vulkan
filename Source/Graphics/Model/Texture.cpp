...
namespace texture::utils {
    ...
    bool load_texture_from_memory(const uint8_t* data, uint32_t width, uint32_t height, VkImage& outImage, VkDeviceMemory& outImageMemory);
...
Texture::Texture(const uint8_t* data, uint32_t width, uint32_t height)
    : _Path("(embedded)"), _Image(VK_NULL_HANDLE), _Memory(VK_NULL_HANDLE), _ImageView(VK_NULL_HANDLE), _Sampler(VK_NULL_HANDLE)
{
    if (!texture::utils::load_texture_from_memory(data, width, height, _Image, _Memory)) {
        throw std::runtime_error("Failed to load embedded texture from memory.");
    }

    _ImageView = texture::utils::createImageView(_Image, VK_FORMAT_R8G8B8A8_SRGB);
    if (_ImageView == VK_NULL_HANDLE) {
        throw std::runtime_error("Failed to create image view for embedded texture.");
    }
...
namespace texture::utils {
    ...
    bool load_texture_from_memory(const uint8_t* data, uint32_t width, uint32_t height, VkImage& outImage, VkDeviceMemory& outImageMemory) {
        if (!data || width == 0 || height == 0) {
            std::cerr << "Invalid data provided for embedded texture." << std::endl;
            return false;
        }
        ...
        stagingBuffer.Map();
        std::memcpy(stagingBuffer.GetData(), data, (size_t)imageSize);
        stagingBuffer.Unmap();
        ...
        Application::Get()->GetDevice().CreateImageWithInfo(
            imageInfo,
