...
void Keyboard_Movement::MoveInPlaneXZ(GLFWwindow* window, float dt, GameObject& gameObject)
{
	glm::vec3 rotate{ 0 };
	...
	if (glm::dot(rotate, rotate) > std::numeric_limits<float>::epsilon())
		gameObject.GetTransform().rotation += _lookSpeed * dt * glm::normalize(rotate);
	...
	if (glm::dot(moveDir, moveDir) > std::numeric_limits<float>::epsilon())
		gameObject.GetTransform().position += _moveSpeed * dt * glm::normalize(moveDir);
}
